...
Read together they shape every architectural decision. The canonical example: the black-tree-bug (resolved 2026-05-05) lived in the unnamed gap between `update()` and `render()`; once the producer/consumer contract was named (the `cachedFrame_` stamp), the bug couldn't recur. **Coda (2026-06-22):** it *did* recur — the later R2b "skip static-natural update" perf path added a bare `continue` that bypassed the stamp, silently re-opening the gap (trees vanished on NVIDIA). Fixed by stamping in the skip path AND adding a registry-side regression guard (`stale_after_drawn`, opt-in-fatal) — nifty `07a1f8ac`, `memory/r2b-skip-reintroduced-cachedframe-gap.md`. Lesson reinforced: a named contract only holds if every new bypass path honors it; back the contract with a guard that makes a violation *observable*, not just documented.
