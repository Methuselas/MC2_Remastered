...
/***************************************************************
 * FILENAME: View.cpp
 * DESCRIPTION: Standalone Viewer host for the Mechlopedia runtime.
 *
 * AUTHOR: Microsoft
 * CREATED: Unknown
 *
 ...
 *
 * CHANGES:
 * - Added Viewer ImGui initialization/render/shutdown calls.
 * - Added legacy GOS underlay visibility control for ImGui FIT testing.
 * - Routed Viewer application name through ViewerVersion helpers.
 ***************************************************************/
...
void __stdcall UpdateRenderers()
{
	...
	if (ViewerImGui_IsLegacyUiVisible())
	{
		gos_SetRenderState( gos_State_Filter, gos_FilterNone );
		pMechlopedia->render();
	}
	...
	// Keep cursor rendering on the game cursor path, not the OS cursor.
	gos_SetRenderState( gos_State_Filter, gos_FilterNone );
	userInput->render();
...
void __stdcall DoGameLogic()
{
	...
	if (ViewerGuiFit_ConsumeCloseRequested())
		quitGame = true;
...
void __stdcall InitializeGameEngine()
{
	...
	ViewerImGui_Initialize(Environment.screenWidth, Environment.screenHeight);
...
void __stdcall GetGameOSEnvironment( const char* CommandLine )
{
	Environment.applicationName			= ViewerVersion_GetApplicationName();
	...
	Environment.version					= ViewerVersion_GetSemVer();
