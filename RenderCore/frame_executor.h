...
// 5 PostProcess sub-stage applies + 6 top-level applies (TerrainDecal,
// TerrainOverlay, StaticPropOpaque, MechOpaque, Water, Shadow).
enum class ApplyPassId : uint8_t {
    ...
    Water,        // APPLY-STATE-WATER-1: top-level water pipeline apply (body-site post-compute-dispatch)
    Shadow,       // APPLY-STATE-SHADOW-1: first render-target-MODE apply -- ShadowMech base pipeline + DepthForwardZ clear at the dynamic shadow seam
    Count,
};
...
inline const char* applyPassName(ApplyPassId id) {
    switch (id) {
        ...
        case ApplyPassId::Shadow:                  return "Shadow";
