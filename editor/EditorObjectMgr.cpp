...
//-------------------------------------------------------------------------------------------------
// The two world-space endpoints of a wall-like object: its center +/- the
// bounding radius along its facing axis. The facing-axis convention (which way
// rotation=0 points, and sin/cos assignment) is the one thing we can only verify
// visually, so it is isolated here for easy calibration.
static void wallEndpoints( const Stuff::Vector3D& pos, float rotDeg, float radius,
                           Stuff::Vector3D& e0, Stuff::Vector3D& e1 )
{
	// Wall long axis. (sin,cos) put the endpoints on the SHORT axis -> walls
	// snapped side-to-end and formed T/cross junctions; the long axis is the
	// perpendicular (cos,-sin).
	const float r = rotDeg * 0.01745329252f;   // deg -> rad
	const float dx = cosf( r );
	const float dy = -sinf( r );
	e0 = pos; e0.x -= dx * radius; e0.y -= dy * radius;
	e1 = pos; e1.x += dx * radius; e1.y += dy * radius;
}

bool EditorObjectMgr::findWallEndpointSnap( const Stuff::Vector3D& ghostPos, float ghostRotDeg,
                                            float ghostRadius, const EditorObject* exclude,
                                            Stuff::Vector3D& outSnappedPos )
{
	const float snapDist = 64.0f;            // wu (~half a 128 cell)
	float bestSq = snapDist * snapDist;
	bool found = false;
	Stuff::Vector3D bestTarget, bestGhostEnd;

	Stuff::Vector3D g0, g1;
	wallEndpoints( ghostPos, ghostRotDeg, ghostRadius, g0, g1 );
	const Stuff::Vector3D ghostEnds[2] = { g0, g1 };

	for ( BUILDING_LIST::EIterator iter = buildings.Begin(); !iter.IsDone(); iter++ )
	{
		EditorObject* o = (*iter);
		if ( !o || o == exclude || !o->appearance() )
			continue;
		if ( getSpecialType( o->getID() ) != WALL )
			continue;

		ObjectAppearance* a = o->appearance();
		Stuff::Vector3D e0, e1;
		wallEndpoints( a->position, a->rotation, a->getRadius(), e0, e1 );
		const Stuff::Vector3D wallEnds[2] = { e0, e1 };

		for ( int i = 0; i < 2; ++i )
			for ( int j = 0; j < 2; ++j )
			{
				const float ddx = wallEnds[i].x - ghostEnds[j].x;
				const float ddy = wallEnds[i].y - ghostEnds[j].y;
				const float d2 = ddx * ddx + ddy * ddy;
				if ( d2 < bestSq )
				{
					bestSq = d2;
					found = true;
					bestTarget = wallEnds[i];
					bestGhostEnd = ghostEnds[j];
				}
			}
	}
	...
	// Translate the ghost so its snapping endpoint lands on the neighbour endpoint.
	outSnappedPos = ghostPos;
	outSnappedPos.x += bestTarget.x - bestGhostEnd.x;
	outSnappedPos.y += bestTarget.y - bestGhostEnd.y;
	return true;
}
