...
1. **ENCYCLO-3D black previews** — mechs/vehicles/BUILDINGS/WEAPONS all black on fresh boot, same path ⇒ SHARED texture source not initialized pre-mission (suspect: TGL-texture FST mount deferred by smart-load). Chain proven healthy (select→variant→pObject→verts in-bounds lit-grey→tris into FBO 8, nonBg=0 ⇒ black texture samples). Probes committed (tgl.cpp drawOldWay vtx dump, [ENCYCLO] select). NEXT: add probe logging `theShape->listOfTextures[i].gosTextureHandle` + node name + one File::open result at preview draw; fresh-boot vs post-mission diff; fix = mount/init the cold source in Mechlopedia::begin next to LogisticsData::init(). Grep what mission load mounts that initTGLForLogistics doesn't (FastFile mounts, smart-load gates: MC2_SMART_LOAD arc).
