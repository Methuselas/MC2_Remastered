...
|---|---|---|---|---|
| 1 | `GameOS/gameos/gos_postprocess.cpp:37` | `MC2_SHADOW_CSM : master gate, DEFAULT OFF. OFF => legacy single dynamic map path is byte-identical` | Code at `:45` is `!(v && v[0]=='0' && v[1]=='\0')` — **DEFAULT ON**; only `=0` disables. Default-ON since 2026-06-18 (`8ff13a36`); `docs/tier1_env_vars.md:103` correctly documents ON. Only the header comment is stale. | lighting / CSM |
| 2 | `GameOS/gameos/gos_postprocess.h:199` | `Gate (MC2_SHADOW_CSM, default OFF). When OFF the legacy single-map path…` | Same gate as #1 — **default ON**. Second stale site for the same claim. | lighting / CSM |
