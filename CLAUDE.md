...
- **TODO: options menu writes bad ResolutionX/Y to options.cfg** — opening the options dialog (even just to change audio) re-saves `options.cfg` and can write a non-800×600 `ResolutionX/ResolutionY` (observed 4096×2160 on a 4K desktop, 2026-05-06). The engine UI canvas is authored against 800×600 and self-scales; any other value here breaks HUD scale, aspect ratio, and video positioning in-mission. Fix: either (a) options dialog must always write `ResolutionX=800 ResolutionY=600` regardless of desktop res, or (b) decouple "render canvas resolution" (800×600) from "presentation resolution" (desktop) so the dialog can store actual desktop res for fullscreen toggle without breaking the canvas. Diagnostic: `grep -i resolution A:/Games/mc2-opengl/mc2-win64-v0.3/options.cfg` — must read 800/600. See `memory/options_cfg_resolution_drift.md` for full triad.
