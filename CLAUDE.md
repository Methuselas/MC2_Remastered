...
- **drawPass-retirement decal static-bake (`MC2_TERRAIN_INDIRECT_OVERLAY`, default-OFF): Slice A sheet bug FIXED 2026-05-17, user-visual-confirmed.** Root cause was unconditional all-map draw feeding the unguarded `1.0/clip4.w` in `terrain_overlay.vert` (off-screen verts -> non-cullable frustum-spanning sheets); fixed with a `px.z in [0,1)` clip-safe guard (no-op for the CPU-pz-culled live path). Remaining (next session, all teed up, DO NOT re-derive): demote `decal_corner_probe`, validate the already-implemented Slice B drawPass-loop conjunction gate, substitutive total-frame Tracy proof, then ship. Full state: `memory/drawpass_retirement_decal_bake_state_and_raster_sheet_trap.md`.
