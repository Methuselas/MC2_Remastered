...
- **Stage 0.5 §4 (renderVisible repoint) BLOCKED on readback non-superset.** §2.5 sticky-bit shipped (commit `91b6991`, retires K-window META-FIX of `056c365`) but §3 precondition canary on mc2_10 measured 2.814% sustained / 30.308% worst — 5-15x over the 0.5%/2% gate. Cause is structural: sticky widens indirect-bucket admit only, readback writes raw frustum at `gpu_cull.comp:239` BEFORE the temporal else-if runs. Pre-existing Task 7 envelope (10-60% dropout) documented at `code/terrobj.cpp:214-225` and was the reason `aeceb2c` was reverted via `e0ea027`. §4 deferred to alpha-Stage 1 readback-quality. v3 spec has POSTSCRIPT noting this. DO NOT attempt to land §4 on HEAD. Full reasoning: `memory/stage_0_5_section_4_blocked_on_readback_non_superset.md`.
