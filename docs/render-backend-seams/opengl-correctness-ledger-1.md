...
| OMT-1 | overlay/decal batch texture bind | M | HIGH (bind-0 sampling driver-defined) | FIXED `36d6a254` | bound GL name 0 on resolve-fail → incomplete texture (driver-defined). Now `bindBatchTextureOrFallback()` binds explicit 1×1 magenta fallback + once-per-handle log + counters; success path byte-unchanged | tier1 5/5 + mc2_01/24 trace PASS, resolve_failed=0 | none. COVERAGE NOTE: legacy CPU overlay batches carry 0 pushes on stock (GPU-indirect owns overlays) → guard hardens a reachable-but-not-default path; fallback not positively exercised by stock smoke |
