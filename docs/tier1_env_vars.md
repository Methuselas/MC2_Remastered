...
- `MC2_DRAW_PACKET_STATIC_PROP_V6=1` — master gate for DrawPacket v6 canonical packet+meta array dispatch. Builds `v6Packets[]` + `v6Meta[]` per frame, validates lockstep invariants, then dispatches via `glDrawElementsInstancedBaseVertexBaseInstance`. Falls through to v5 (if enabled) or legacy multidraw on disarm or lockstep violation. Emits `[DRAW_PACKET_V6]` to stderr: `event=armed` once, `event=dispatch_summary` every 600 frames (`slots_considered`, `draws_issued`, `zero_instance_skips`, `sorted_oob`, `type_oob`, `lockstep_violations`, `gl_errors`, `ok`). `ok=1` iff all error counters are 0 and `draws_issued+zero_instance_skips==slots_considered`. Default OFF. Cached at process start.
- `MC2_DRAW_PACKET_STATIC_PROP_V6_TRACE=1` — per-slot verbose trace for v6. Emits one `[DRAW_PACKET_V6] slot=S pkt=P type=T group=G inst=I base=B drawID=D first=F count=C baseV=V` line per issued draw. Requires `MC2_DRAW_PACKET_STATIC_PROP_V6=1`. Default OFF. Cached at process start.
