...
- `MC2_SHADOW_DYNAMIC_PROP_CASTERS=1` — opt-in (SHADOW-DYNAMIC-PROP-CASTERS-1): feed the **dynamic** sun-shadow caster pass from the **full registry** (`GpuStaticPropRegistry::getDynamicPropShadowInstances` — ALL non-building props, visibility-independent) instead of the camera-visible `s_typeRanges` set, which only admitted props near the camera (so distant trees never cast into the now-correctly-camera-fit dynamic map; the projection fix `a365e6ad` exposed this). When ON, `flushShadow` is skipped for static props (registry replay replaces it) and buildings keep casting via the static map — so **pair with `MC2_STATIC_PROP_BUILDING_SHADOW`**. Requires `MC2_SHADOW_ENABLE`. Default **OFF**. `=2` adds a `[SHADOW_DYN_PROP v1] recipes_in/types/inst/draws` trace. **Debt:** no light-box cull yet (draws all map props every frame; HZB planned).
