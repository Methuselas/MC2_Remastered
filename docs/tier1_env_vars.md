...
- `MC2_SNAPSHOT_MECH_EXTRACT=1` — MECH-EXTRACTION-1 observe-only mech snapshot (default OFF). Wires `ExtractedMechPacket[]` from the persist buffer populated in `flush()` before `s_pendingSubmits.clear()`. All counters informational; NOT in `ok` gate. Explicit forced-ON canary: run `--mission mc2_24 --duration 30` with this set and verify `[mech-extract]` line shows `snapshot=6 mat_valid=6 mat_sentinel=0 count_mismatch=0 handle_mismatch=0 objectid_mismatch=0 tex_mismatch=0 mat_mismatch=0` by frame 3. Implementation: `gos_mech_batcher.cpp` + `render_snapshot.cpp`. Independent materialIdx compare via `s_mechHandleToMaterialIdx` (same map used for SSBO upload, valid post-flush until next flush).
