...
**Last refreshed:** 2026-05-17 (LightsData UBO->SSBO ceiling removed `b41baec`; static-actor mission-load lighting bake shipped behind `MC2_LIGHTBAKE` kill-switch `2db2a04` — substitutive proof PENDING user capture; drawPass-retirement Slice A+B shipped — substitutive proof confirmed; raster-triangle Fix A + Fix B shipped; VertexProjectLoop retirement plan drafted).
