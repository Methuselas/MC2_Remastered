...
>
> **Formula / uniform notes.** CPU `distanceToEye` used `GetApproximateLength` (octagonal approx) after a length-preserving camera-frame transform; the GLSL `length()` is exact — this is an intentional, parity-correct upgrade (the binary clamp is identical when `MinHazeDistance == MaxClipDistance == 50000`, the stock values). The camera haze constants (`Camera::MinHazeDistance` / `MaxClipDistance` / `DistanceFactor`) are runtime mission state, so the new `g_minHazeDistance` / `g_maxClipDistance` / `g_distanceFactor` uniforms MUST be uploaded as live per-frame uniforms, **not baked** at mission load.
>
