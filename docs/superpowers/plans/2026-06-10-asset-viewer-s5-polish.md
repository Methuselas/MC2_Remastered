...
- Override key `appearanceName` = the `FileName=` field inside `data/tgl/{X}.ini` (`bdactor.cpp:340/357` staticProp, `:4087/4100` tree). **Roster source = unique `FileName` values across `<deploy>/data/tgl/*.ini`.**
