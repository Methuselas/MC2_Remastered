...
- [ ] **Step 1:** In each of the 4 water sub-blocks, wrap ONLY the (ii) statements so legacy runs when the fast path does NOT own the armed draw: `if (!gos_terrain_indirect::WaterFastPathOwnsArmedDraw()) { <wx..ww writes>; <the if(clipped1||clipped2){ handle-resolution + addTriangleBulk x2 }> } else { /* armed: GPU water path owns the draw; skip legacy enqueue */ }`. **The gate is PER-STATEMENT, NOT a contiguous range (GROUNDING CONCERN-1, spec-review verified):** follow the exact per-statement KEEP/GATE tags GROUNDING Step 1 recorded for each sub-block. GATE-(ii) = ONLY the `wx/wy/wz/ww` writes + the `if(clipped1||clipped2)` body (handle-resolution + the two `addTriangleBulk`). KEEP UNCONDITIONAL (outside the gate, exactly per GROUNDING tags): the `projectForTerrainAdmission` call, the `clipData`/`isVisible` logic, the `clipInfo = clipData` if/else pair (M2a; 8 branch points across 4 blocks - none gated), the `leastZ/mostZ/leastW/mostW/leastWY/mostWY` 6-tuple min/max, the `else { waterHandle=0xffffffff; waterDetailHandle=0xffffffff; }` sentinel(s) (sonnet MAJOR-2 - always written, never stale), **and critically `calcThisFrame |= 2` (GROUNDING ~:1063/1130/1197/1264) which sits physically BETWEEN the GATE-(ii) `wx..ww` writes and the KEEP-(i) 6-tuple - it MUST stay UNCONDITIONAL (CONCERN-1: sweeping it into the gate breaks the per-vertex projection self-skip and diverges cross-frame state from legacy on un-armed frames).** Because `calcThisFrame|=2` is interleaved between gated `wx..ww` and kept 6-tuple, the gate is two (or more) small per-statement wraps within each sub-block, not one block wrap - encode exactly the boundaries GROUNDING enumerated.
