...
**Advisor scope-change note — parity gate weakened (RESOLVED 2026-05-08):** The design spec (Section 6) calls for dual-FBO `MC2_MECH_GPU_PARITY=1` automated zero-mismatch parity as the Slice A gate. This plan intentionally defers dual-FBO parity to Slice A+ and substitutes operator visual observation plus `[MECHBATCHER v1] event=summary fallback_total=0` as the Slice A gate. **Advisor sign-off received 2026-05-08** — same precedent as renderWater Stage 2+3, indirect-terrain SOLID PR1, and PR2c mine static-bake (all shipped clean with operator-visual + counter-zero gates). Slice A+ adds dual-FBO parity, scheduled as a hard gate before any default-on flip OR if operator visual misses a regression during the 7-day soak. Conditions and post-soak requirements documented in `docs/superpowers/plans/progress/2026-05-08-track-d-execution-prompt.md`.
