#!/usr/bin/env python3
"""check-raw-gl-depthmask.py -- RAW-GL-DEPTHMASK-DIFF-GATE-1 (Phase 8 enforcement).

North-star Phase 8 SECOND enforcement gate. Bans NEW raw glDepthMask() call sites
...
chokepoint (applyPipeline / GlScopedDepthState).

Exact clone of scripts/check-raw-gl-depthfunc.py, swapped to the glDepthMask axis.
...
bans the raw CALL SITE itself, pushing depth-write-mask state through the
sanctioned emitter rather than scattered glDepthMask() calls. Non-overlapping
mechanism.
...
Two modes:
  * Mode A (default, ENFORCEMENT): inspect only the lines ADDED in a diff vs a
    base ref. Any newly-added raw glDepthMask( whose file is NOT in the allowlist
  ...
  * Mode B (--all, AUDIT): walk the render TUs and list EVERY existing raw
    glDepthMask( site with file:line + per-file counts. Advisory, exit 0 --
...
Exit code: Mode A -> 1 if any new unsanctioned raw glDepthMask, else 0.
           Mode B -> always 0.
"""
import argparse
import os
...
# bare glDepthMask(  (allow whitespace after the name, before the paren)
DEPTHMASK_RE = re.compile(r"\bglDepthMask\s*\(")

SRC_EXT = ("*.cpp", "*.h", "*.hpp", "*.cc", "*.cxx")

# ---------------------------------------------------------------------------
# ALLOWLIST -- sanctioned files that MAY contain glDepthMask(. Only NEW raw
...
ALLOWLIST_EXACT = {
    # applyPipeline emitter -- the sanctioned chokepoint for depth-mask state.
    "GameOS/gameos/pipeline_binder.cpp",
    # GlScopedDepthState RAII wrapper (mask, func).
...
# FROZEN BACKLOG (v1): existing raw glDepthMask sites, allowlisted by EXACT file
...
def mode_a(base, quiet=False):
    """Scan ADDED diff lines for new raw glDepthMask( in non-allowlisted files."""
    r = git(["diff", base, "--"] + list(SRC_EXT))
    if r.returncode != 0:
        print("[check-raw-gl-depthmask] ERROR: git diff failed: %s"
    ...
    for line in r.stdout.splitlines():
        ...
        if not line.startswith("+"):
            continue
        added = line[1:]
        if not DEPTHMASK_RE.search(strip_comments(added)):
            continue
    ...
    if not quiet:
        print("[check-raw-gl-depthmask] Mode A (enforcement) -- base %s" % base)
    if not uniq:
        if not quiet:
            print("[check-raw-gl-depthmask] OK: no newly-added raw glDepthMask "
                  "outside sanctioned sites")
        return 0
    for fpath, text in uniq:
        print("RAW-GL-DEPTHMASK-NEW %s    %s" % (fpath, text))
    print("[check-raw-gl-depthmask] FAIL: %d new raw glDepthMask call(s) outside "
          "the sanctioned allowlist (route depth-mask through applyPipeline / "
          "GlScopedDepthState)" % len(uniq), file=sys.stderr)
    return 1
...
def mode_b():
    """List every existing raw glDepthMask( site (advisory)."""
    ...
    for path in walk_source():
        ...
        for i, raw in enumerate(lines, 1):
            ...
            if DEPTHMASK_RE.search(scan):
                hits.append(i)
        if hits:
            per_file[os.path.relpath(path, REPO_ROOT).replace(os.sep, "/")] = hits

    total = sum(len(v) for v in per_file.values())
    print("[check-raw-gl-depthmask] Mode B (audit) -- frozen raw glDepthMask backlog")
    print("[check-raw-gl-depthmask] total raw glDepthMask( sites: %d across %d file(s)"
          % (total, len(per_file)))
    print("")
    for fpath in sorted(per_file):
        ...
        for ln in lines:
            print("       %s:%d" % (fpath, ln))
    print("")
    print("[check-raw-gl-depthmask] (advisory; exit 0) v1 freezes these files; "
...
def main():
    ...
    ap.add_argument("--all", action="store_true",
                    help="Mode B: list the full raw glDepthMask backlog (advisory)")
